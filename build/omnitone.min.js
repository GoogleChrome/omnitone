...
var Omnitone=function(){"use strict";
/**
   * @license
   * Copyright 2016 Google Inc. All Rights Reserved.
   * Licensed under the Apache License, Version 2.0 (the "License");
...
/**
   ...
   * limitations under the License.
   */
const iA={getBrowserInfo:function(){const A=navigator.userAgent;let t,e=A.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*([\d.]+)/i)||[];if(/trident/i.test(e[1]))return t=/\brv[ :]+(\d+)/g.exec(A)||[],{name:"IE",version:t[1]||""};if("Chrome"===e[1]&&(t=A.match(/\bOPR|Edge\/(\d+)/),null!=t))return{name:"Opera",version:t[1]};e=e[2]?[e[1],e[2]]:[navigator.appName,navigator.appVersion,"-?"],null!=(t=A.match(/version\/([\d.]+)/i))&&e.splice(1,1,t[1]);let i=A.match(/android|ipad|iphone/i);return i||(i=A.match(/cros|linux|mac os x|windows/i)),{name:e[0],version:e[1],platform:i?i[0]:"unknown"}},patchSafari:function(){window.webkitAudioContext&&window.webkitOfflineAudioContext&&(window.AudioContext=window.webkitAudioContext,window.OfflineAudioContext=window.webkitOfflineAudioContext)}},nA={};return nA.browserInfo=iA.getBrowserInfo(),nA.createBufferList=function(A,t,e){return new y(A,t,e||{dataType:"url"}).load()},nA.mergeBufferListByChannel=A.mergeBufferListByChannel,nA.splitBufferByChannel=A.splitBufferByChannel,nA.createFOAConvolver=function(A,t){return new L(A,t)},nA.createFOARouter=function(A,t){return new H(A,t)},nA.createFOARotator=function(A){return new R(A)},nA.createHOARotator=function(A,t){return new V(A,t)},nA.createHOAConvolver=function(A,t,e){return new Y(A,t,e)},nA.createFOARenderer=function(A,t){return new U(A,t)},nA.createHOARenderer=function(A,t){return new eA(A,t)},A.log(`Version 1.4.0 (running ${nA.browserInfo.name} ${nA.browserInfo.version} on ${nA.browserInfo.platform})`),"safari"===nA.browserInfo.name.toLowerCase()&&(iA.patchSafari(),A.log(`${nA.browserInfo.name} detected. Polyfill applied.`)),nA}();
